...
from _tex_transform import _transform_template_tex, _write_full_spec
...
def generate_latex_file(template_dir, journal_name=None, output_dir=None):
    ...
    tex_struct = None
    if files['tex']:
        tex_struct = _transform_template_tex(files['tex'][0], target_journal)

    # 生成完整 .tex 文件 - 强制包含所有模板要求的段落和排版规格注释
    print('[1/5] 生成完整 .tex 文件...')
    tex_content = _build_complete_tex(cls_info, tex_struct, target_journal, layout_spec)
