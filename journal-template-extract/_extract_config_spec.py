...
def _safe_bib_resource(value):
    ...
    if (not text or '...' in text or '<' in text or '>' in text or
            any(token in lowered for token in ('your ', 'filename', 'bibdatabase', 'bib file')) or
            re.search(r'[<>:"|?*]', text)):
        return ''
    return text
...
def _extract_biblatex_config(self):
    ...
    for m in re.finditer(BS + r'addbibresource\{([^}]+)\}', self.content):
        bib_resource = _safe_bib_resource(m.group(1))
        if bib_resource:
            spec.setdefault('bibresources', []).append(bib_resource)
