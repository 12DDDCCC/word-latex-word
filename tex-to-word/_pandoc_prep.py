...
def _strip_pandoc_unsupported_layout_primitives(content):
    """Drop LaTeX layout hints/placeholders that Pandoc cannot parse."""
    result = content
    # Pandoc does not understand TeX vertical-adjust primitives; Word
    # pagination is rebuilt later, so these template page hints are dropped.
    result = re.sub(r'\\vadjust\s*\{[^{}]*\}', '', result)
    # Some publisher templates leave draft placeholder art such as
    # \centerline{\includegraphics[...,draft]{empty}. It is not manuscript
    # content, and incomplete braces make Pandoc fail before conversion.
    result = re.sub(
        r'(?m)^\s*\\centerline\s*\{\s*\\includegraphics\s*\[[^\]]*\bdraft\b[^\]]*\]\s*\{empty\}\s*\}?\s*$',
        '',
        result,
    )
    result = re.sub(
        r'(?m)^\s*\\includegraphics\s*\[[^\]]*\bdraft\b[^\]]*\]\s*\{empty\}\s*$',
        '',
        result,
    )
    return result
...
def _protect_tikz(result, start_index=0):
    ...
    """
    tikz_table_pattern = (
        r'\\begin\{(table\*?)\}(.*?)\\begin\{tikzpicture\}'
        r'(.*?)\\end\{tikzpicture\}(.*?)\\end\{\1\}'
    )
...
def prepare_tex_for_pandoc(tex_path, work_dir, bbl_path=None):
    ...
    result = content
    result = _strip_pandoc_unsupported_layout_primitives(result)
