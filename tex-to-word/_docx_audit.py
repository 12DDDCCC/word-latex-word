...
def audit_docx_structure(docx_path):
    ...
    result['tblpPr_count'] = len(root.findall(f'.//{{{_W}}}tblpPr'))
...
def _collect_relationships(rels_xml):
    ...
    try:
        ...
        return rel_ids, image_rels
    for rel in rels_root.findall(f'{{{_R}}}Relationship'):
        rid = rel.get('Id')
