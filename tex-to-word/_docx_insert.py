...
def _apply_float_page_top(insert_anchor, block_elems):
...
def _float_page_height_mm(doc, style_spec):
    ...
    return max((section.page_height - section.top_margin - section.bottom_margin) / 36000.0, 120.0)
...
def _float_column_width_mm(doc, style_spec):
    ...
    section = doc.sections[0]
    return max((section.page_width - section.left_margin - section.right_margin) / 36000.0, 40.0)
...
def _float_delay_target_mm(doc, style_spec, required_space_mm=None):
...
def _full_width_table_anchor(anchor_elem, doc, table_elem, style_spec):
...
def _full_width_float_insert_anchor(anchor_elem, doc=None, style_spec=None, required_space_mm=None):
    ...
    target_mm = 0.0 if legacy_one_paragraph else _float_delay_target_mm(
        doc, style_spec, required_space_mm)
    filled_mm = 0.0
    ...
    while candidate is not None:
        if candidate.tag == qn("w:p"):
            ...
            if not text or _is_empty_section_break_para(candidate):
                ...
                continue
            style_id = _paragraph_style_id(candidate).lower()
            if style_id.startswith("heading") or _is_float_placeholder_text(text):
                break
            if _is_caption_like_text(text):
                break
...
def postprocess_docx(docx_path, display_formula_data, tables, cite_map=None, cite_style='apa', bbl_path=None, layout_spec=None, style_spec=None, tex_path=None):
    ...
    if cite_map and _HAS_CROSSREF:
        if bbl_path and os.path.exists(bbl_path):
            ...
            build_references_section(doc, bbl_items, cite_map=cite_map)
...
def _latex_image_width_to_docx(width_spec, section, image_path=None, compiled_layout=None):
    ...
    elif section.page_width and section.left_margin is not None and section.right_margin is not None:
        max_width = section.page_width - section.left_margin - section.right_margin
    else:
        max_width = Inches(6.5)
    if section.page_height and section.top_margin is not None and section.bottom_margin is not None:
        text_height = section.page_height - section.top_margin - section.bottom_margin
    else:
        text_height = Inches(9.0)
    ...
    if compiled_layout.get("columnwidth"):
        ...
        col_width = (max_width - (col_count - 1) * col_space) // col_count if col_count > 1 else max_width
...
def embed_images_in_docx(docx_path, images, tex_dir, layout_spec=None, style_spec=None):
    ...
    for i, img_info in enumerate(images):
        ...
        if is_full_width:
            _apply_float_page_top(insert_anchor, block_elems)
