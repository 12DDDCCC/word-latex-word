...
def copy_support_files(template_result, bib_path, output_dir, docx_path=None, skeleton_info=None):
    ...
    # bib 文件名从模板骨架动态提取
    bib_filename = (skeleton_info or {}).get('bib_filename', 'references')
...
def _run_latex_steps(tex_name, stem, output_dir, timeout=120):
    steps = ['xelatex', 'bibtex', 'xelatex', 'xelatex']
...
def compile_tex(tex_path, output_dir):
    """xelatex + bibtex 编译（分步执行，每步打印进度）

    使用相对路径避免中文路径导致 XeLaTeX 崩溃。
    """
    tex_path = Path(tex_path)
    output_dir = Path(output_dir)
    ...
    try:
        ...
        ok = _run_latex_steps(tex_name, stem, output_dir)
