...
from skeleton_builder import (
    ...
    _build_preamble_from_spec, class_options_from_spec, sanitize_bib_filename,
)
...
def _class_option_accepted(option, declared_options):
    declared = set(declared_options or [])
    if not declared:
        return bool(option)
    return bool(option) and ('*' in declared or option in declared)
...
def _safe_graphics_extensions(value):
    ...
    if not parts or any(re.fullmatch(r'\.[A-Za-z0-9]+', part) is None for part in parts):
        return ''
    return ','.join(parts)
...
def _safe_graphics_path(value):
    text = str(value or '').strip()
    lowered = text.lower()
    if (not text or '...' in text or '<' in text or '>' in text or
            any(token in lowered for token in ('your ', 'filename', 'path/to'))):
        return ''
    return text


def _normalize_elsarticle_3p(options, evidence, declared):
    """归一化 elsarticle 的 twocolumn+manuscript → 3p。

    elsarticle 的 manuscript（preprint）模式配合 twocolumn 视觉不安全
    （标题/摘要排版错乱），3p 才是其真正的双栏机制。无论 spec 是否
    提取成功，只要确认是 elsarticle 且 evidence 含 twocolumn+manuscript，
    就归一化为 3p，保证产物一致。
    """
    if (
        'elsarticle' in [str(o).lower() for o in evidence]
        and 'manuscript' in evidence
        and 'twocolumn' in evidence
        and _class_option_accepted('3p', declared or ['1p', '3p', '5p'])
    ):
        options = [opt for opt in options if opt not in ('twocolumn', 'manuscript', 'preprint')]
        if '3p' not in options:
            options.append('3p')
        print('  [class-options] elsarticle twocolumn/manuscript normalized to 3p')
    return options


def _normalize_class_options_for_spec(spec, journal, doc_options=None):
    """Return legal, template-aware documentclass options.

    The extracted spec is the authority.  elsarticle is the one deliberate
    special case: twocolumn+manuscript is visually unsafe in preprint mode,
    while 3p uses elsarticle's real two-column title/abstract machinery.
    """
    requested = [str(opt).strip() for opt in (doc_options or []) if str(opt).strip()]
    dc = spec.get('document_class', {}) if spec else {}
    declared = set(dc.get('declared_options', []) or []) if spec else set()
    defaults = [str(opt).strip() for opt in (dc.get('default_options', []) or [])
                if str(opt).strip()] if spec else []
    evidence = list(dict.fromkeys([*requested, *defaults]))
    class_name = str(dc.get('class_name') or journal or '').lower()

    if not spec:
        # spec 提取失败：仍对 elsarticle 做 3p 归一化，保证产物一致
        # evidence 需补入 journal/class_name 以便后续判断
        evidence = list(dict.fromkeys([*requested, str(journal or '').lower()]))
        return _normalize_elsarticle_3p(list(requested), evidence, declared)

    config_mode = next(
        (opt for opt in requested if opt in ('final', 'manuscript', 'discussions')),
        None,
    )
    options = class_options_from_spec(spec, journal, config_mode)
    evidence = list(dict.fromkeys([*requested, *defaults, *options]))

    # elsarticle twocolumn+manuscript → 3p 归一化（spec 齐全分支）
    options = _normalize_elsarticle_3p(options, [*evidence, class_name], declared)

    invalid = []
    if declared and '*' not in declared:
        invalid = [
            opt for opt in evidence
            if not _class_option_accepted(opt, declared)
        ]
    if invalid:
        print(f'  [class-options] ignored unsupported option(s): {", ".join(invalid)}')

    legal = [opt for opt in options if _class_option_accepted(opt, declared)]
    ...
    if not table_layout.get('rule_style'):
        if table_format.get('rule_style'):
            table_layout['rule_style'] = table_format['rule_style']
                ...
                or {'toprule', 'colrule', 'botrule'}.issubset(hline_commands):
...
def assemble_tex(text_result, image_result, table_result,
    ...
    normalized_doc_options = (
        _normalize_class_options_for_spec(spec, template_result['journal'], doc_options)
        if spec else list(dict.fromkeys(doc_options or []))
    )
    ...
    template_numbering = detect_template_numbering_mode(
        template_dir_path, layout_spec, doc_options=normalized_doc_options)
    ...
    if spec:
        ...
        cls_options = normalized_doc_options
    ...
    if normalized_doc_options:
        _mode = None
        for _opt in normalized_doc_options:
    ...
    bib_filename = sanitize_bib_filename(
        bib_spec_d.get('bib_filename', '') or skeleton_info.get('bib_filename', 'references')
    )
...
def _apply_spec_to_preamble(preamble, spec, layout_spec, spec_pkgs):
    ...
    graphics_path = _safe_graphics_path(fig_fmt.get('graphics_path'))
    if graphics_path and '\\graphicspath' not in preamble:
        preamble += f'\\graphicspath{{{graphics_path}}}\n'
    graphics_extensions = _safe_graphics_extensions(fig_fmt.get('graphics_extensions'))
    if graphics_extensions and '\\DeclareGraphicsExtensions' not in preamble:
        preamble += f'\\DeclareGraphicsExtensions{{{graphics_extensions}}}\n'
    ...
    # 参考文献字号：elsarticle 等 cls 设了 \bibfont 却不在 thebibliography 中调用，这里强制生效
    bib_size = bib_fmt.get('font_size', '')
    if bib_size and '\\renewcommand{\\bibfont}' not in preamble:
        preamble += f'\\renewcommand{{\\bibfont}}{{\\{bib_size}}}\n'
