...
def sanitize_bib_filename(value, default='references'):
    """Return a safe bibliography database stem for LaTeX and filesystem use."""
    ...
    if raw.lower().endswith('.bib'):
        raw = raw[:-4]
    lowered = raw.lower()
    placeholder_tokens = ('your', 'bibdatabase', 'bib database', 'bibfile',
                          'bib file', 'filename', 'database')
    if not raw or raw in ('.', '..') or any(token in lowered for token in placeholder_tokens):
        return default
...
def parse_template_skeleton(template_content):
    ...
    if bib_file_match:
        info['bib_filename'] = sanitize_bib_filename(bib_file_match.group(1))
